...
class ViewController: UIViewController, CBCentralManagerDelegate, CBPeripheralDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    var getBtn: UIButton? = UIButton.init()
...
//        cutLinkBtn?.frame = CGRect(x: (view.frame.width - 80)/2, y: button!.frame.origin.y + 130, width: 80, height: 80)
...
//        getBtn?.frame = CGRect(x: (view.frame.width - 80)/2, y: stopBtn!.frame.origin.y + 100, width: 80, height: 80)
//        getBtn?.setTitle("设备列表", for: .normal)
//        getBtn?.setTitleColor(UIColor.white, for: .normal)
//        getBtn?.backgroundColor = UIColor.blue
//        getBtn?.layer.cornerRadius = 10
//        getBtn?.addTarget(self, action: #selector(deviceList), for: .touchUpInside)
//        self.view.addSubview(getBtn!)
        
        //创建表格图
        self.tableView = UITableView(frame: CGRect(x: 40, y: stopBtn!.frame.origin.y + 80, width: self.view.bounds.width - 80, height: self.view.bounds.height - 200), style: .plain)
        //将代理，数据来源设为自己
        self.tableView?.delegate = self
        self.tableView?.dataSource = self
        //创建表头标签
        let headerLabel = UILabel(frame: CGRect(x: 40, y: stopBtn!.frame.origin.y + 80, width: self.view.bounds.width - 80, height: 30))
...
//       peripheralArray.append(peripheral.name as Array)
    }
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        if peripheral.name != nil {
            ...
            peripheralArray.append(peripheral.name!)
            print("搜索可连接的蓝牙设备为：\(peripheralArray.count)")
            print("搜索可连接的蓝牙设备为====：\(peripheralArray)")
            ...
            if peripheral.name == "Allway Halo 20 APP" {
                ...
                // 连接外设成功或者失败分别会进入回调方法
                // MAKE: -连接外设
//                peripheralArray.add(peripheral.name)
...
//        //
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.textLabel?.text = peripheralArray[indexPath.row]
        return cell!
    }
